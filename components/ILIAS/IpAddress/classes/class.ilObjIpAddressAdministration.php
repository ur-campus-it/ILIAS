...
final class ilObjIpAddressAdministration extends ilContainer
{
    ...
    private static function loadRootOrgRefIdAndId(): void
    {
        if (self::$root_ref_id === -1 || self::$root_id === -1) {
            ...
            $res = $db->query("SELECT o.obj_id, r.ref_id FROM object_data o
			INNER JOIN object_reference r ON r.obj_id = o.obj_id
			WHERE title = '__IpAddressAdministration'
			LIMIT 1")->fetchAssoc();
    ...
    {
        if (parent::getTitle() !== "__IpAddressAdministration") {
            return parent::getTitle();
        } else {
            return $this->lng->txt("objs_ipad");
        }
    }

    public static function _exists(int $id, bool $reference = false, ?string $type = null): bool
    {
        return parent::_exists($id, $reference, "ipaa");
    }
